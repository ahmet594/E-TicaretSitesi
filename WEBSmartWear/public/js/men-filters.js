...
console.log("Men-filters.js yüklendi");
...
let activeFilters = {
    category: 'Erkek', // Ana kategori (değişmez)
...
function initMenFilters() {
    console.log("Men filters init başlatılıyor...");
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM yüklendi, erkek sayfası hazırlanıyor...");
...
async function fetchErkekProducts() {
    showLoading();
    
    try {
        console.log("Erkek ürünleri yükleniyor...");
        const response = await fetch('/api/products/category/Erkek');
        
        if (!response.ok) {
            throw new Error('Erkek ürünleri yüklenirken hata oluştu: ' + response.status);
        }
...
function updateCategoryCounters() {
    ...
    allProducts.forEach(product => {
        ...
        if (product.subcategory) {
            const subcategoryKey = 'erkek-' + product.subcategory.toLowerCase().replace(/\s+/g, '-');
        ...
        if (product.type) {
            const type = 'erkek-' + product.type.toLowerCase().replace(/\s+/g, '-');
...
function mapHtmlCategoryToDbCategory(htmlCategory) {
    // HTML'deki erkek- ön ekini temizle
    const cleanCategory = htmlCategory.replace('erkek-', '');
    
    // Özel eşleştirmeler
    const categoryMappings = {
        'kot-pantolon': ['kot-pantolon', 'kot', 'jean', 'jeans', 'denim'],
        'kumaş-pantolon': ['kumaş-pantolon', 'kumaş'],
        'tshirt': ['tshirt', 't-shirt', 'tişört'],
        'gömlek': ['gömlek', 'gomlek'],
        'ceket': ['ceket'],
        'kaban-parka': ['kaban', 'parka', 'kaban-parka'],
        'kazak': ['kazak', 'sweater'],
        'mont': ['mont', 'jacket'],
        'sweatshirt': ['sweatshirt', 'hoodie'],
        'takım-elbise': ['takım-elbise', 'takım', 'elbise', 'suit'],
        'şortlar': ['şort', 'şortlar', 'short'],
        'hırka': ['hırka', 'hirka', 'cardigan'],
        'yelek': ['yelek', 'vest'],
        'eşofman': ['eşofman', 'esofman', 'tracksuit'],
        'cüzdan': ['cüzdan', 'cuzdan', 'wallet']
    };
...
function applyFilters() {
    ...
    // Start with all products (always men's products)
    filteredProducts = [...allProducts];
    console.log("Toplam erkek ürünleri:", filteredProducts.length);
...
function updateActiveFiltersTags() {
    ...
    if (activeFilters.subcategory) {
        ...
        addFilterTag('Kategori', subcategoryName, () => {
            ...
            currentCategoryTitle.textContent = 'Tüm Erkek Ürünleri';
            applyFilters();
            updateUrlParams();
        });
    }
...
window.filterByCategory = function(category) {
    ...
    if (category === 'erkek-kot-pantolon') {
